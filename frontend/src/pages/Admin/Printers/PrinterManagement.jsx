...
export default function PrinterManagement() {
  ...
  useEffect(() => {
    ...
    printerService
      .getAll()
      .then((res) => {
        if (!active || !Array.isArray(res?.data) || !res.data.length) return;
        const mapped = res.data.map((p, idx) => ({
          id: p.id ? (p.id.length > 8 ? `PRN-${p.id.slice(0, 4).toUpperCase()}` : p.id) : `PRN-${String(idx + 1).padStart(3, '0')}`,
          name: p.name,
          model: p.model || 'Standard',
          location: p.location || 'Station',
          connection: 'Wi-Fi',
          status: p.status === 'online' ? 'Online' : p.status === 'maintenance' ? 'Printing' : 'Offline',
        }));
        setPrinters(mapped);
      })
      .catch((err) => {
        console.warn('Using local printers list:', err.message);
      });
    return () => {
      active = false;
    };
  ...
  const handleAddPrinter = async (event) => {
    ...
    try {
      ...
      console.warn('Could not save printer to backend:', err.message);
    }
