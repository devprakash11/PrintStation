...
export default function AdminAuthSecure() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (mode === 'forgot') {
      if (!email) {
        setMessage({ type: 'error', text: 'Enter your admin email address.' });
        return;
      }
      ...
      try {
        ...
        setMessage({
          type: 'success',
          text: response.message || 'If an active admin account exists, a reset link has been sent.',
        });
      } catch (error) {
        setMessage({ type: 'error', text: error.message || 'Unable to send the reset link. Please try again.' });
      } finally {
        setLoading(false);
      }
      return;
    ...
    if (mode === 'reset') {
      if (!resetToken) {
        setMessage({ type: 'error', text: 'The reset link is missing or invalid.' });
        return;
      }
      ...
      try {
        ...
        setMessage({ type: 'error', text: error.message || 'This reset link is invalid or expired.' });
      } finally {
        setLoading(false);
      }
      return;
    ...
    try {
      if (mode === 'signup') {
        ...
        const createdRole = response?.data?.user?.role;
        setMessage({
          type: 'success',
          text: response?.message || (createdRole === 'admin'
            ? 'Administrator account created successfully.'
            : 'Account created successfully. You have staff access.'),
        });
      } else {
  ...
  return (
    <main className="admin-auth-page">
      ...
      <section className="admin-auth-shell" aria-label="Admin authentication">
        <Link className="admin-auth-back" to={isReset ? '/admin' : '/'}>
          <ArrowLeft size={16} />
          {isReset ? 'Back to login' : 'Back to home'}
        </Link>
        ...
        <div className="admin-auth-card">
          ...
          <div className="admin-auth-heading">
            ...
            <h1>
              {isLogin ? 'Welcome back.' : isSignup ? 'Create your account.' : isForgot ? 'Forgot password?' : 'Set a new password.'}
            </h1>
            <p>
              {isLogin
                ? 'Sign in to manage your PrintStation printer portal.'
                : isSignup
                  ? 'Create a PrintStation account. The first account receives administrator access; later accounts receive staff access.'
                  : isForgot
                    ? 'Enter your admin email and we will send you a secure password reset link.'
                    : 'Choose a new password for your PrintStation admin account.'}
            </p>
          </div>
          ...
          <form className="admin-auth-form" onSubmit={handleSubmit} noValidate>
            ...
            {message.text && (
              <p className={`admin-auth-message is-${message.type}`} role="alert">
                {message.text}
              </p>
            )}
          ...
          {isLogin && (
            <button type="button" className="admin-auth-forgot" onClick={() => switchMode('forgot')}>
          ...
          {(isForgot || isReset) && (
            <p className="admin-auth-switch">
              <button type="button" onClick={() => switchMode('login')}>Return to Login</button>
            </p>
          )}

          <p className="admin-auth-note">
            Authentication is handled by the PrintStation backend. Passwords are never stored in the browser or repository.
          </p>
        </div>
      </section>
    </main>
