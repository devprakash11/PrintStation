...
export default function Users() {
  ...
  useEffect(() => {
    ...
    userService
      ...
      .catch((err) => {
        console.warn('Using local users list:', err.message);
      });
    return () => {
      active = false;
    };
  ...
  const handleSubmit = async (event) => {
    ...
    if (editingUser) {
      ...
      let createdId = `USR-${String(users.length + 1).padStart(3, '0')}`;
      try {
        ...
        console.warn('Backend create failed, saving locally:', err.message);
      }
      saveUsers([{ id: createdId, name, email, role: form.role, status: form.status, jobs: 0, lastActive: 'Never' }, ...users]);
      setPage(1);
    }
    closeModal();
  };
  ...
  const handleToggleStatus = async (user) => {
    ...
    try {
      ...
      console.warn('Backend status toggle failed:', err.message);
    }
