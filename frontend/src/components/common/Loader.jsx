...
export default function Loader({ label = 'Loading...', fullScreen = true }) {
  ...
  useEffect(() => {
    const handleLoading = (event) => {
      setVisible(Boolean(event.detail?.active));
    };
  ...
  return (
    <div className={fullScreen ? 'app-loader app-loader--fullscreen' : 'app-loader'} role="status" aria-live="polite" aria-label={label}>
      <style>{`
        .app-loader {
          display: inline-flex;
          align-items: center;
