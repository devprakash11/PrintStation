...
export const authService = {
  async login({ email, password }) {
    ...
    if (res?.data?.token && res?.data?.user) authService.saveSession(res.data.user, res.data.token);
    return res;
  },
  ...
  async logout() { authService.clearSession(); },
  saveSession(user, token) {
    if (token) localStorage.setItem(TOKEN_KEY, token);
    if (user) localStorage.setItem(SESSION_KEY, JSON.stringify({ id: user.id, name: user.name, email: user.email, role: user.role, status: user.status }));
  },
