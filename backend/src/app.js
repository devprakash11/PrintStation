...
app.use(cors({origin:(origin,cb)=>!origin||origins.includes(origin)?cb(null,true):cb(new Error('CORS origin denied')),credentials:true}));
...
app.get('/api/v1/health',async(req,res,next)=>{try{await query('SELECT 1');res.json({success:true,status:'ok',service:'printstation-api',timestamp:new Date().toISOString()});}catch(e){next(e);}});
app.get('/api/v1',(_,res)=>res.json({success:true,name:'PrintStation API',version:'2.0.0'}));
app.use((err,req,res,next)=>{console.error(err);if(err.name==='ZodError')return res.status(400).json({success:false,message:'Invalid request.',issues:err.issues});if(err.code==='23505')return res.status(409).json({success:false,message:'A record with these values already exists.'});if(err.code==='23503')return res.status(400).json({success:false,message:'Referenced record does not exist.'});if(err.code==='LIMIT_FILE_SIZE')return res.status(413).json({success:false,message:'File is too large.'});res.status(err.status||500).json({success:false,message:env.nodeEnv==='production'?'Internal server error.':err.message});});
export default app;
